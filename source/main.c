...
#include "menu.h"			// CommandMenu_Option, MENU_*
...
const static CommandMenu_Strings tPhrases = {
	.MenuPrefix = NULL,
	.MenuSuffix = "Choice: ",
...
#define __TRAIN_COMMANDMENU(NAME, ...)												\
	static const CommandMenu_Option NAME##Menu[] = {							\
		__VA_ARGS__,																\
		{""},																		\
		{"Return", MENU_RETURN},													\
		{"Exit", MENU_EXIT}															\
...
void DecelerateTrain(SystemHandle hSystem, SystemUnsignedData uSpeed)
{
	Train.ChangeSpeed(hSystem, -(SystemSignedData)uSpeed);
}

void StopTrain(SystemHandle hSystem)
{
	Train.SetSpeed(hSystem, 0);
}
...
__TRAIN_COMMANDMENU_COMPONENT(Engine,
	...
	{""},
	{"set absolute speed", MENU_UDATA, {Engine.SetSpeed, Engine.AbsoluteSpeedMax}},
...
static const CommandMenu_Option StartMenu[] = {
